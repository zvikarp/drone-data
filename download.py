...
def get_arguments():
    ...
    parser.add_argument('-d', '--download-folder', type=str, default="data/ulg/",
...
def main():
    ...
    if args.print_entries:
        ...
        logfile_pattern = os.path.join(
            os.path.abspath(args.download_folder), "*.ulg")
        logfiles = glob.glob(os.path.join(os.getcwd(), logfile_pattern))
        logids = frozenset(os.path.splitext(
            os.path.basename(f))[0] for f in logfiles)
            ...
            for num_tries in range(100):
                try:
                    if args.overwrite or entry_id not in logids:
                        ...
                        print(
                            'downloading {:}/{:} ({:})'.format(i + 1, n_en, entry_id))
